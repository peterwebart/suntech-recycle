...
/* Shared: "every service here" grid */
function ServicesAvailable({ where }: { where: string }) {
  return (
    <Section tone="light">
      <Container>
        <div data-reveal className="max-w-[60ch]">
          <Eyebrow>Available in {where}</Eyebrow>
          <h2 className="mt-3.5 text-[clamp(24px,3.4vw,34px)]">
            Every service, same chain of custody
          </h2>
        </div>
            ...
            <li key={s.slug} data-reveal>
              ...
              >
                ...
                <Icon
                  name="arrow-right"
                  size={16}
                  className="ml-auto text-green opacity-0 transition-all group-hover:translate-x-0.5 group-hover:opacity-100"
                />
              </Link>
            </li>
          ))}
...
function CountryView({ loc }: { loc: Location }) {
  const crumbs = [
    { name: "Home", path: "/" },
    { name: "Service areas", path: "/locations" },
    { name: loc.name, path: `/locations/${loc.slug}` },
  ];
  const all = citiesInCountry(loc.countrySlug);
    ...
    <>
      ...
      <section className="bg-near-black py-[clamp(40px,6vw,72px)] text-white">
        <Container>
          ...
          <h1 className="mt-3 max-w-[20ch] text-[clamp(32px,5vw,54px)]">
            Electronics recycling &amp; ITAD in {loc.name}.
          </h1>
      ...
      <Section tone="light">
        <Container>
          ...
          <div data-reveal className="mt-10">
            <h2 className="text-[clamp(22px,3vw,30px)]">Cities we serve in {loc.name}</h2>
          </div>
          ...
          {neighbourhoods.length > 0 && (
            <div data-reveal className="mt-10">
              <h3 className="text-[18px]">Montréal neighbourhoods</h3>
      ...
      <ServicesAvailable where={loc.name} />
      <PageCta title={`Retire hardware anywhere in ${loc.name}`} />
    </>
  );
}

/* ── City / neighbourhood page ────────────────────────────────────────────── */
function CityView({ loc }: { loc: Location }) {
  const countryEntry = getLocation(loc.countrySlug);
  const crumbs = [
    { name: "Home", path: "/" },
    { name: "Service areas", path: "/locations" },
    ...(countryEntry
      ? [{ name: countryEntry.name, path: `/locations/${countryEntry.slug}` }]
      : []),
    { name: loc.name, path: `/locations/${loc.slug}` },
  ];
  ...
  const siblings = citiesInCountry(loc.countrySlug)
  ...
  return (
    <>
      ...
      <section className="bg-near-black py-[clamp(40px,6vw,72px)] text-white">
        <Container>
          ...
          <h1 className="mt-3 max-w-[22ch] text-[clamp(32px,5vw,54px)]">
            Electronics recycling &amp; data destruction in {loc.name}.
          </h1>
          <p className="mt-5 max-w-[58ch] text-[clamp(16px,2vw,19px)] text-[#d6e5db]">
            {loc.intro}
          </p>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
            ...
            >
              ...
              <p className="flex items-center gap-2 border-t border-line px-5 py-3 font-mono text-[11.5px] tracking-[0.04em] text-ink-soft">
                ...
                {loc.isHeadquarters
                  ? `Our processing facility — ${site.address.street}`
                  : loc.localService
                    ? `${loc.name} coverage · orange marks our Longueuil facility`
                    : `${loc.name}, ${loc.region}`}
              </p>
            </div>

            <div data-reveal>
              <Eyebrow>{loc.served ? "Areas served" : "How we serve you"}</Eyebrow>
              <h2 className="mt-3 text-[clamp(22px,3vw,30px)]">Coverage in {loc.name}</h2>
              {loc.served ? (
                <>
                  <p className="mt-3 text-[16px] text-ink-soft">
                    Neighbourhoods and sectors we regularly collect from:
                  </p>
                  <ul className="mt-4 flex flex-wrap gap-2.5">
                    {loc.served.map((area) => (
                      <li
                        key={area}
                        className="rounded-full border border-line bg-paper px-3.5 py-2 text-[13.5px] font-medium text-ink"
                      >
                        {area}
                      </li>
                    ))}
                ...
                <ul className="mt-4 grid gap-3">
                  {[
                    loc.localService
                      ? "Fast, free on-site collection for qualifying volumes"
                      : "Project-based on-site collection for larger volumes",
                    "Secure, fully trackable mail-in service for drives & IT assets",
      ...
      {loc.reviews && loc.reviews.length > 0 && (
        <Section tone="paper">
          <Container>
            <Reviews reviews={loc.reviews} cityName={loc.name} />
          </Container>
        </Section>
      )}

      <WhyChoose place={loc.name} tone={loc.reviews ? "light" : "paper"} />
      <ServicesAvailable where={loc.name} />
          ...
          <Container>
            <div data-reveal className="flex flex-wrap items-end justify-between gap-4">
              <h2 className="text-[clamp(22px,3vw,30px)]">
                Other areas in {countryEntry?.name ?? "the region"}
              </h2>
              <Link
                href={countryEntry ? `/locations/${countryEntry.slug}` : "/locations"}
      ...
      <PageCta
        title={`Electronics recycling & data destruction in ${loc.name}`}
        body={`Request a collection or a secure destruction quote. Call ${site.phone.display} — local collection in Greater Montréal and secure service ${loc.country === "Canada" ? "across Canada" : `in ${loc.country}`}.`}
      />
    </>
  );
}

/* Small breadcrumb wrapper to keep hero markup tidy. */
...
export default async function LocationPage({ params }: Params) {
  ...
  const loc = getLocation(city);
  if (!loc) notFound();
  return loc.kind === "country" ? <CountryView loc={loc} /> : <CityView loc={loc} />;
}
